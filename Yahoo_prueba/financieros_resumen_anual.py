...
SUMMARY_TABLE   = os.environ.get("SUMMARY_TABLE", f"{SNOWFLAKE_DATABASE}.{SNOWFLAKE_SCHEMA}.FIN_SUMMARY_MAIN")
FIN_BATCH_TICKERS = int(os.environ.get("FIN_BATCH_TICKERS", "40"))  # nº tickers por lote

TZ = ZoneInfo("Europe/Madrid")
CURRENT_YEAR = datetime.now(TZ).year
START_YEAR = int(os.environ.get("SUMMARY_START_YEAR", "2021"))
END_YEAR   = int(os.environ.get("SUMMARY_END_YEAR", str(CURRENT_YEAR - 1)))  # sólo años cerrados
...
# ================== Utils de extracción (anual, compacto) ==================
...
EQUITY_KEYS    = ["Stockholders Equity", "Total Equity Gross Minority Interest"]  # fallback si no, calculamos
...
def summarize_by_year(ticker: str) -> pd.DataFrame:
    tk = yf.Ticker(ticker)
    balance = tk.balance_sheet        # anual
    income  = tk.financials           # anual
    cf      = tk.cashflow             # anual

    # Unir columnas (periodos) de todas las tablas
    cols = set()
    ...
    if not cols:
        return pd.DataFrame(columns=[
            "TICKER","YEAR","ASSETS","LIABILITIES","EQUITY","REVENUE","EXPENSES",
            "NET_INCOME","OPERATING_CF","INVESTING_CF","FINANCING_CF","FREE_CF",
            "DEBT_EQUITY","ROA","ROE","NET_MARGIN"
        ])

    rows = []
    # Recorre periodos (más reciente primero)
    for col in sorted(cols, key=lambda x: pd.Timestamp(x), reverse=True):
        ...
        rows.append({
            ...
            "FINANCING_CF": fin_cf,
            "FREE_CF": free_cf,
            "DEBT_EQUITY": float(round(debt_equity, 6)) if debt_equity is not None else None,
            "ROA": float(round(roa, 6)) if roa is not None else None,
            "ROE": float(round(roe, 6)) if roe is not None else None,
            "NET_MARGIN": float(round(net_margin, 6)) if net_margin is not None else None,
        })
    ...
    df = pd.DataFrame(rows).drop_duplicates(subset=["TICKER","YEAR"])
...
def upsert_summary(conn, df: pd.DataFrame):
    ...
    WHEN MATCHED THEN UPDATE SET
      t.ASSETS = s.ASSETS, t.LIABILITIES = s.LIABILITIES, t.EQUITY = s.EQUITY,
      t.REVENUE = s.REVENUE, t.EXPENSES = s.EXPENSES, t.NET_INCOME = s.NET_INCOME,
      t.OPERATING_CF = s.OPERATING_CF, t.INVESTING_CF = s.INVESTING_CF,
      t.FINANCING_CF = s.FINANCING_CF, t.FREE_CF = s.FREE_CF,
      t.DEBT_EQUITY = s.DEBT_EQUITY, t.ROA = s.ROA, t.ROE = s.ROE,
      t.NET_MARGIN = s.NET_MARGIN
    WHEN NOT MATCHED THEN
      INSERT (TICKER,YEAR,ASSETS,LIABILITIES,EQUITY,REVENUE,EXPENSES,NET_INCOME,
              OPERATING_CF,INVESTING_CF,FINANCING_CF,FREE_CF,DEBT_EQUITY,ROA,ROE,NET_MARGIN,OP_MARGIN)
      VALUES (s.TICKER,s.YEAR,s.ASSETS,s.LIABILITIES,s.EQUITY,s.REVENUE,s.EXPENSES,s.NET_INCOME,
              s.OPERATING_CF,s.INVESTING_CF,s.FINANCING_CF,s.FREE_CF,s.DEBT_EQUITY,s.ROA,s.ROE,s.NET_MARGIN)
    """
    with conn.cursor() as cur:
        cur.execute(merge_sql)
        conn.commit()
    print(f"Upsert FIN_SUMMARY: {nrows} filas")
...
if __name__ == "__main__":
    ...
    try:
        ...
        for batch in chunked(tickers, FIN_BATCH_TICKERS):
            ...
            for tck in batch:
                try:
                    df = summarize_by_year(tck)
                    if not df.empty:
                        all_rows.append(df)
                except Exception:
                    # evita que un ticker bloquee el lote
                    pass
                time.sleep(0.05)
        if all_rows:
        ...
        print("✅ Resumen financiero anual actualizado.")
    finally:
        conn.close()
